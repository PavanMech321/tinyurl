...
router.post('/links', async (req, res) => {
  ...
  const ins = await query(
    'INSERT INTO links (code, url) VALUES ($1, $2) RETURNING code, url, created_at, last_clicked_at, total_clicks',
    [finalCode, url]
  );
...
router.get('/links', async (req, res) => {
  const { q, sort = 'created_at', dir = 'desc' } = req.query as any;
...
router.get('/links/:code', async (req, res) => {
  ...
  const r = await query(
    'SELECT code, url, created_at, last_clicked_at, total_clicks FROM links WHERE code = $1',
    [code]
  );
